...
On peu voir le code correspondant à ces essais dans la fonction \texttt{simd\_test} du listing de code \ref{annexe:sum-int}. Cependant, je n'ai pas pu généré des tâches suffisamment longues avec cette méthode car je rencontrait un problème de mémoire avant d'avoir un temps d'éxécution suffisamment long. Je n'ai pas pu identifier pouquoi on rencontrait ce problème si tôt, et je n'ai donc pas pu utiliser cette méthode pour générer des tâches avec des temps d'exécution différents.