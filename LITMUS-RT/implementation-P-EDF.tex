...
Pour montrer le fonctionnement de cet algorithme, si l'on se place sur un même \gls{processeur}, on peut visualiser l’exécution de deuxtâche periodiques : 
\begin{figure}[H]
    \center
    ...
    \caption{Exemple de EDF à 2tâches}
\end{figure}

On a ici une premièretâche $\tau_1$ avec un pire temps d'éxécution (\textit{Worst Case Execution Time}) de 2 et une période de 5, et une secondetâche $\tau_2$ avec un pire temps d'éxécution de 4 et une période de 15. On a alors préemption de la $\tau_2$ à $t=5$ afin d'éxécuter $\tau_1$. Cela est dû au réveil de la tâche $\tau_1$ (représenté par la flêche montante) et à la date d'échéance plus proche de cette dernière. 
