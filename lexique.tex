\newglossaryentry{ordonancement}{%
    name = {ordonancement},
    description = {}
}
\newglossaryentry{ordonanceur}{%
    name = {ordonanceur},
    description = {}
}
\newglossaryentry{processeur}{%
    name = {processeur},
    description = {Ca c'est la définition}
}
\newglossaryentry{plateforme heterogene}{%
    name = {plateforme hétérogène},
    description = {Système formé d'un ensemble de processeurs différents}
}
\newglossaryentry{SOC}{%
    name = {SOC},
    description = { ou \textit{Système On a Chip} est }
}

\newglossaryentry{cluster}{%
    name = {cluster},
    description = {ensemble interconnecté de plusieurs processeurs}
}

\newglossaryentry{bootloader}{%
    name = {bootloader},
    description = {court programme chargé au démarrage de l'ordinateur initialisant le système d'exploitation}
}
\newglossaryentry{git}{%
    name = {git},
    description = {systèùe de gestion de versuibns décentralisé, utilisé pour suivre les modifications apportées à des fichiers sources dans un projet de développement logiciel}
}